...
import { showExplainEffect, explainState } from "./explain";
...
const createTooltipForSelection = (state: EditorState): readonly Tooltip[] => {
  ...
  if (isQuickEditActive || isExplainActive) {
    return [];
  }
  ...
  return [
    {
      ...
      create() {
        ...
        const explainButtonText = document.createElement("span");
        explainButtonText.textContent = "Explain";

        const explainButtonShortcut = document.createElement("span");
        explainButtonShortcut.textContent = "⌘E";
        explainButtonShortcut.className = "text-sm opacity-60";

        explainButton.appendChild(explainButtonText);
        explainButton.appendChild(explainButtonShortcut);
...
const selectionTooltipField = StateField.define<readonly Tooltip[]>({
  ...
  update(tooltips, transaction) {
    ...
    for (const effect of transaction.effects) {
      if (effect.is(showQuickEditEffect) || effect.is(showExplainEffect)) {
        return createTooltipForSelection(transaction.state);
      }
    }
    return tooltips;
